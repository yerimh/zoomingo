...
1. For each of the required features, where should we look for its implementation? Reference the numbers from the section below along with the file name and line numbers . For example:
    ...
    ```
2. (No more than 1 paragraph / 4 sentences) What would you have done, fixed, or added if you had more time?
...
2. Start on the backend, getting the basic structure of the endpoints working (correct names and methods, parameter validation, returning some placeholder data). Then create the database and one-by-one start replacing placeholder data with data from the db.

3. Choose one of the backend endpoints (e.g., `/bingo`) and starting on its frontend components (e.g., the `#bingo` button) implement it completely. Then, choose the next simplest endpoint (e.g., `selectScenarios`) and do the same.
...
Your project must match the behavior of the screencast provided here. It should not _exactly_ match pixel-for-pixel what's shown here style-wise, but the behavior **must** follow the described specifications and behavior.

* [Screencast of Required Behavior](https://courses.cs.washington.edu/courses/cse154/20au/homework/fp/zoomingo.mp4)

Note that the styling applied to this example is very minimal, this sample implementation gives you a sense of expected behavior. You are **required** to not copy this styling and instead, gives your final project a different (and more exciting) look.
...
2. If the user has not yet selected a size or the game was reset, the board is empty. After a size is selected, the board is **always** displayed as a grid with equal number of cells on each side (e.g., a 5x5 grid).
...
3. Clicking the "New Game" button populates the board with scenarios, fetched from the backend. (A "scenario" is the contents of each cell in the board. For example: "Can you hear me now?")
    ...
    * When the `#new-game` button is disabled when a new game starts. It is re-enabled on `#reset`

4. All boards must contain a "Free" scenario, and it must be in the middle of the board.
    * The "Free" scenario always has a scenario ID of 1. IDs are unique, and no other scenario can have the ID of 1.

5. Clicking the "Reset" button (with ID `reset`) clears the board and clears data in LocalStorage.
    ...
    * `#reset` also re-enables the `#name` input text field

6. Scenarios can be clicked on, and are styled to indicate that they have been selected.
    * All elements with the class `scenario` have a click event listener.
    * When a scenario is clicked, the `/selectScenarios` endpoint is called, passing the scenario ID.
      * From above, You should be able to get the scenario ID by reading the id of the `scenario` element that was clicked on.
    ...
    * **Note**: Scenarios may have duplicate text. For example, a player may see more than one "Dog ate my webcam" squares on their board. Despite the text being the same, all scenarios on the board should have unique IDs. As such, selecting one will only select that one, not others with the same text.

7. Clicking a "Bingo" button calls the `/bingo` API to determine if the current player won.
...
8. A new `game_id` is created for every new game. Every unique player name also has a `player_id`.
...
9. The list of possible scenarios and the list of known players are each stored in separate database tables server-side.
...
10. The entire game state is stored and updated in a database table server-side.
...
11. When the page first loads, LocalStorage is checked for a `game_id` and `player_id`.
...
* Use sql joins to relate data between tables in the `zoomingo` database.
...
* all possible errors need to be appropriately handled, returning the correct error codes and reasonable messages.
...
considered a violation of our course [academic integrity](https://courses.cs.washington.edu/courses/cse154/20au/syllabus/syllabus.html#academic-conduct)
...
If we find [inappropriate content or plagiarism](https://courses.cs.washington.edu/courses/cse154/20au/syllabus/index.html#academic-conduct) in projects **you will be ineligible
for any points on the project**. Ask the instructors if you're unsure if your work is cited appropriately. Any external sources like images should be cited where used in the source code or (ideally) visible in a page footer. Refer to this [copyright example](https://courses.cs.washington.edu/courses/cse154/20au/code-examples/assets/copyright-examples/copyrightexample2.html) page for how to cite images from different sources.